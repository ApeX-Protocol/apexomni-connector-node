...
export class PrivateApi {
  ...
  async createOrder(params: CreateOrderOptions): Promise<OrderObject> {
    ...
    if (!params.expiration) {
      params.expiration = Date.now() + 30 * 24 * 60 * 60 * 1000;
    }
    if (!signature) {
      ...
      if (params.isSetOpenTp) {
        ...
        // @ts-ignore
        const tpExpiration = params.expiration;
      ...
      if (params.isSetOpenSl) {
        ...
        // @ts-ignore
        const slExpiration = params.expiration;
  ...
  async openOrders(): Promise< OrderObject[] > {
    return this.request('/open-orders', 'get', {});
  }

  async openAllOrders(showPositionOrders: boolean): Promise<OrderObject[] > {
    return this.request('/open-orders', 'get', {showPositionOrders});
  }
